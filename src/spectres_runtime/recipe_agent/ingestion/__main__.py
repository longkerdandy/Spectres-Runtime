...
Idempotent (design §7): re-running rewrites the corpus over itself with no duplicate
...
def main() -> int:  # pragma: no cover - one-shot ingestion glue; needs a live DB + embedder
    ...
    Builds the shared ``Knowledge`` handle (DB + embedder) from ``Settings``, drains
